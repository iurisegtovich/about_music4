...
Program music
  ...
  Use ff, Only: ff_Info, ff_init, ff_display
  ...
  Use ffcall, Only: ffcall_init , ffcall_nrg, ffcall_force
  Implicit None
  ...
  Call ff_init(ffmodel,ctrl_filename,scell,species)
  Call ff_display(ffmodel,2,dUnit)
  Call ffcall_init()
  !----------------------------------------------------------
  ...
  simno =nsims
  mtime = 0.0_RDbl
  Write(*,*) "Going to read: ", genparams%niterations ," configfs"
  nconfs=0
  Do iter = 1, genparams%niterations
    !read position form xyz file
    Call config_xyz2config(species,scell,xyzfile,success,.false.)
    If (.Not.success) Exit

    nconfs=nconfs+1
    Call stopcriteria_check(species,restartfile)

    ! main force field call
!    Call ffcall_nrg(imodel,scell,species,"INTRA",intranrg)
!    Call ffcall_nrg(imodel,scell,species,"NCOUL",intranrg)
!    Call ffcall_nrg(imodel,scell,species,"COUL",intranrg)
    Call ffcall_nrg(ffmodel,scell,species,"TOTAL",intranrg)
!    Call ffcall_force(imodel,scell,species,force)

    !** Dump to std io
    If (Mod(iter, genparams%iprint) == 0) Then
      Write(dunit,*) "ITER< TOTNRG: ", iter, intranrg
    Endif

    !** Dump to the config file
    If (Mod(iter, genparams%iconfig) == 0) Then
      Call datafile_writeconfig(species,configfile,mtime)
    End If
