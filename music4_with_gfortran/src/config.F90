!#Music -- Multipurpose Simulation Code:  Object-oriented molecular 
!#simulation software
...
Module config
  ...
  Public :: AtMolCoords, RectCoords, default_config_tag, &
                                !------------ DISPLAY ------------------------
      config_display, &
      ...
      config_getnatoms, config_getconfig, config_getaccel, &
                                ! -------- INIT routine -----------
      config_defaultinit, config_initdefaultsize,  config_checkinit, &
      ...
      config_setxyz, config_copy, config_copysubset,&
      config_rotatexyz, config_getq, &
      ...
      config_reverseVels, config_systemKE, config_makeimages, &
      ...
      config_xyz2molec, config_setindices, config_swapatoms, &
      config_displaybyindex, config_r2rp, config_config2xyzSeparate, &
      config_indextospc
  ...
  Type AtMolCoords
    ...
    Type(VecType), Dimension(:,:), Pointer    :: forces  !"fast" accl.
    Real(rdbl), Dimension(:,:), Pointer       :: charges
    Integer, Dimension(:,:),Pointer :: index  ! each atom is given a unique number
...
Contains
  ...
  Subroutine config_defaultinit(sorbates)
    ...
    Do sorb = 1, nsorbates
      ...
      If (config_isfixed(sorbates(sorb))) Then
        ...
        Nullify(sorbates(sorb)%forces)
        Nullify(sorbates(sorb)%index)
        Nullify(sorbates(sorb)%charges)
  ...
  Subroutine config_allocfields(species, spc, nmoles, init_gc)
    ...
    Allocate(species%afast(natoms, nmoles),  &
        species%forces(natoms, nmoles),&
        species%index(natoms, nmoles),&
        species%charges(natoms, nmoles),&
        species%aslow(natoms, nmoles),&
        STAT=error)
    If (error /= 0) Then
      Write(0,'(1x,2a,i4,a)') __FILE__," : ",__LINE__, &
          " Could not allocate 'species%afast, aslw, forces'"
      Stop
    End If

    !** Initialize all the fields 
    Do a = 1,natoms
      species%charges(a,1:nmoles) = molecules_getcharge(spc,a)
      Do m = 1,nmoles
    ...
    Do spc = 1,nspc
      ...
      Do mol = 1,nmols
        Do atm = 1,natms
          ...
          image(spc)%forces(atm,mol) = old(spc)%forces(atm,mol)
          image(spc)%index(atm,mol) = old(spc)%index(atm,mol)
          image(spc)%charges(atm,mol) = old(spc)%charges(atm,mol)
  ...
  Subroutine config_checkandincr(species, spc, nmoles, optPreserve)
    ...
    Else If (nmoles >= nsize) Then
      ...
      Nullify(species(spc)%forces)
      Nullify(species(spc)%index)
      Nullify(species(spc)%charges)
  ...
  Subroutine config_copysubset(orig,dest,osubset,dsubset)
    ...
    Do spc = 1,nspcs
      ...
      If (depth > 1) Then   !** copy molecules, atoms individually
        Do mol = 1,nmols
          ...
          Do atm = 1,natms
            ...
            dest(dspc)%forces(datm,dmol) = orig(spc)%forces(atm,mol)
            dest(dspc)%index(datm,dmol) = orig(spc)%index(atm,mol)
            dest(dspc)%charges(datm,dmol) = orig(spc)%charges(atm,mol)
      ...
      Else   !** copy the whole species in bulk to speed process
        ...
        dest(dspc)%forces(1:natms,1:nmols) = orig(spc)%forces(1:natms,1:nmols) 
        dest(dspc)%index(1:natms,1:nmols) = orig(spc)%index(1:natms,1:nmols) 
        dest(dspc)%charges(1:natms,1:nmols) = orig(spc)%charges(1:natms,1:nmols) 
  ...
  Subroutine config_copy(from_conf , to_conf)
    ...
    Do spc=1,nspc
      !** copy this species
      nmoles=from_conf(spc)%nmoles
      natoms=from_conf(spc)%natoms

      !** Copy all the molecules
      to_conf(spc)%nmoles = nmoles
      to_conf(spc)%natoms = natoms

      !** check array sizes again, see NOTE in header
      If ( (Size(to_conf(spc)%coords,2)<nmoles) .Or. &
          (Size(to_conf(spc)%coords,1)/=natoms ) ) Then
        Write(*,*) "sizes dont match in copysubset"
        Write(*,*) "nmoles-to : ", Size(to_conf(spc)%coords,2), &
            "nmoles from : " ,nmoles
        Write(*,*) "natoms-to : ", Size(to_conf(spc)%coords,1), &
            "natoms from : " ,natoms
        Write(*,'(1x,2a,i4)') __FILE__," : ",__LINE__
        Stop
      Else
        to_conf(spc)%coords(1:natoms,1:nmoles) = &
            from_conf(spc)%coords(1:natoms, 1:nmoles)
        to_conf(spc)%afast(1:natoms,1:nmoles) = &
            from_conf(spc)%afast(1:natoms, 1:nmoles)
        to_conf(spc)%forces(1:natoms,1:nmoles) = &
            from_conf(spc)%forces(1:natoms, 1:nmoles)
        to_conf(spc)%index(1:natoms,1:nmoles) = &
            from_conf(spc)%index(1:natoms, 1:nmoles)
        to_conf(spc)%charges(1:natoms,1:nmoles) = &
            from_conf(spc)%charges(1:natoms, 1:nmoles)
        to_conf(spc)%aslow(1:natoms,1:nmoles) = &
            from_conf(spc)%aslow(1:natoms, 1:nmoles)
        Do m=1,nmoles
          !** Operator overloading here, all fields of gcoords are copied
          to_conf(spc)%gcoords(m) = from_conf(spc)%gcoords(m) 
        End Do
  ...
  !-------------------------------------------------------------------
  ! as of now it swaps %r, %rp and index
  !-------------------------------------------------------------------
  Subroutine config_swapatoms(conf, spc1, m1, a1, spc2, m2, a2)
    Type(AtMolCoords), Dimension(:), Intent(InOut) :: conf

    Integer, Intent(in) ::spc1, m1, a1, spc2, m2, a2
    Call swap(conf(spc1)%coords(a1,m1)%r, conf(spc2)%coords(a2,m2)%r)
    Call swap(conf(spc1)%coords(a1,m1)%rp, conf(spc2)%coords(a2,m2)%rp)
    Call swap(conf(spc1)%coords(a1,m1)%v, conf(spc2)%coords(a2,m2)%v)
    Call swap(conf(spc1)%index(a1,m1), conf(spc2)%index(a2,m2))
    ! charges should not be swapped, they are molecular propoery
    ! at least in the case of EVB 
  End Subroutine config_swapatoms
  ...
  Subroutine config_copymolec(coords1,molec1,coords2,molec2,opt_copy_a)
    ...
    coords2%charges(1:natoms, molec2) = coords1%charges(1:natoms, molec1)

    !** Copy accelerations if desired
    If (copy_accels) Then
      coords2%afast(1:natoms, molec2) = coords1%afast(1:natoms, molec1)
      coords2%forces(1:natoms, molec2) = coords1%forces(1:natoms, molec1)
      coords2%index(1:natoms, molec2) = coords1%index(1:natoms, molec1)
...
!!$  Character(len=strLen) Function config_getsourcefile(sorbates, sorbtype)
  ...
  Subroutine config_getq(species,spc,mol,atm,charge)
    ...
    charge = species(spc)%charges(atm,mol)
  ...
  Subroutine config_zeroforces(sorbate, fast)
    ...
    If (fast) Then
      Do a=1, natoms
        Do m=1, nmoles
          sorbate%afast(a,m) = zero
          sorbate%forces(a,m) = zero
        EndDo
      End Do
    Else
      Do a=1, natoms
        Do m=1, nmoles
          sorbate%aslow(a,m) = zero
          sorbate%forces(a,m) = zero
        EndDo
      End Do
    End If
    ...
    If (fast) Then
      Do m=1, nmoles
        Do a=1, natoms
          ...
          sorbate%forces(a,m) = sorbate%afast(a,m)*atom_mass/scalef
        EndDo
      End Do
    Else
      Do m=1, nmoles
        Do a=1, natoms
          atom_mass = atom_getmass(molecules_getatype(sorbtype, a))
          sorbate%forces(a,m) = sorbate%aslow(a,m)*atom_mass/scalef
        EndDo
      End Do
    End If
  ...
  Subroutine config_conv2accel(species,spc,fast)
    ...
    If (fast) Then
      Do m = 1,nmoles
        Do a = 1,natoms
          species%afast(a,m) = species%forces(a,m)*invmass(a)*scalef
          !Write(*,*)    species%afast(a,m) , species%forces(a,m)
        EndDo
      End Do
    Else
      Do m = 1,nmoles
        Do a = 1,natoms
          species%aslow(a,m) = species%forces(a,m)*invmass(a)*scalef
        EndDo
      End Do
    End If
  ...
  ! xyzbreaks=.true --> means every moleucle has its natoms, and comment line
  ! xyzbreaks=.false --> the whole config is written as a single 
  !  moleucle with one value of natoms and one comment line 
  !-------------------------------------------------------------------------
  Subroutine config_xyz2config(species,scell,filename,success,opt_xyzbreaks)
    Type(AtMolCoords), Dimension(:), Intent(InOut) :: species
    Type(Simcell_Params) :: scell
    Character(len=strLen), Intent(in)              :: filename
    Logical, Intent(out) :: success    
    Logical, Intent(in),Optional :: opt_xyzbreaks    
    Logical :: xyzbreaks
    Integer       :: nsorbs, spc

    xyzbreaks=.True.
    If (Present(opt_xyzbreaks))     xyzbreaks=opt_xyzbreaks
    nsorbs = Size(species)
    Do spc = 1,nsorbs

      !     Write(*,'(1x,2a,2i4)') __FILE__," : ",__LINE__, spc

      If (config_isfixed(species(spc))) Cycle
      Call config_xyz2spc(species, scell, spc, filename,success,xyzbreaks)
  ...
  ! Updates coordinates from an xyz file
  !-------------------------------------------------------------------------
  Subroutine config_xyz2spc(species,scell,spc,filename,success,xyzbreaks)
    Type(AtMolCoords), Dimension(:), Intent(InOut) :: species
    Type(Simcell_Params) :: scell
    Integer, Intent(in)                            :: spc
    Character(len=strLen), Intent(in)              :: filename
    Logical, Intent(out) :: success    
    Logical, Intent(in) :: xyzbreaks    
    Integer       :: nmoles, mol
    success=.True.  ! for nmoles=0 case
    nmoles = config_getnmoles(species, spc)
    Do mol = 1,nmoles
      !      Write(*,'(1x,2a,2i4)') __FILE__," : ",__LINE__, mol
      Call config_xyz2molec(species, scell, spc, mol, filename,&
          success,xyzbreaks)
      If (.Not.success) Return
    End Do
  ...
  Subroutine config_xyz2molec(species, simcell, spc, mol, filename, success, &
    ...
    !      Write(*,'(1x,2a,2i4)') __FILE__," : ",__LINE__, natoms
    !** temp_index is used to avoid an infinte loop here 
    !** Assumes that only MAX_SORBS molecules are there in the file 
if (xyzbreaks) then
    ...
    If (xyz_natoms /= natoms) Then
      ...
      !Write(unitno,*) xyz_natoms 
      Stop
    End If

    species(spc)%natoms = natoms
else
if (nvecs_read==0) then
...
nvecs_Read=nvecs_read+natoms
if (nvecs_read==config_totalvecs(species))  nvecs_Read=0
else
nvecs_Read=nvecs_read+natoms
if (nvecs_read==config_totalvecs(species))  nvecs_Read=0
endif
  ...
  !            spc -- species number
  !----------------------------------------------------------------------------
  Real(Kind=RDbl) Function config_systemKE(species,ke_array)
    Type(AtMolCoords), Intent(In), Dimension(:) :: species 
    Real(rdbl),Dimension(:),Optional, Intent(out) :: ke_array
    Integer         :: spc, nsorbs
    Real(kind=RDbl) :: sumKE, spcKE

    !** Zero the kinetic energy.
    sumKE = zero 

    nsorbs=Size(species,1)
    if (present(ke_array))    ke_array(1:nsorbs)=zero
    Do spc=1,nsorbs
      If (config_isfixed(species(spc))) Cycle
      spcKE=config_kineticEnergy(species,spc)
      sumKe=sumKE+spcKE
      if (present(ke_array))        ke_array(spc)=spcKE
    Enddo
    config_systemKE=sumKE
  End Function config_systemKE




  !----------------------------------------------------------------------------
  ! Assigns particle numbers to each atom
  !----------------------------------------------------------------------------
  Subroutine config_setindices(species)
    Type(AtMolCoords), Intent(InOut), Dimension(:) :: species 
    Integer         :: spc, nspc, m, nmoles, a, natoms, index
    index=0
    ...
    Do spc=1,nspc
      ...
      Do m=1,nmoles
        Do a=1,natoms
          index=index+1
          species(spc)%index(a,m)=index
        end do
      end do
    end do
  End Subroutine config_setindices



  !----------------------------------------------------------------------------
  ! Given an index (for example from a movie.xyz file
! find the spc, mol, amt number
! for now we will assume config_setindices was not called before
  !----------------------------------------------------------------------------
  Subroutine config_indextospc(species, index, spc, mol, atm)
    Type(AtMolCoords), Intent(InOut), Dimension(:) :: species
    Integer , Intent(in) :: index
    Integer , Intent(out) :: spc, mol, atm
 
    Integer         :: s, nspc, m, nmoles, a, natoms, ind
    ind=0
    nspc=molecules_getnsorbs()
    Do s=1,nspc
      nmoles=config_getnmoles(species, s)
      natoms=molecules_getnatoms(s)
      Do m=1,nmoles
        Do a=1,natoms
          ind=ind+1
if (ind==index) then
...
return
  ...
  End Subroutine config_indextospc
...
!!$    End Do
  ...
  Subroutine config_displaycoords(sorbates, sorbno, optunit)
    ...
    Do m=1, nmoles
      Write(unitno, '(2x, a, i6)') "natoms: ", natoms
      Write(unitno, '(2x, a, i6)') "Molecule: ", m
      Do a=1, natoms
        Write(unitno,'(3x,i4, a35)') a,  &
...
!--------------------------------------------------------------------------
! very costly.. maybe atomlist should be stored in a common place
!--------------------------------------------------------------------------
  Subroutine   config_getspcinfoFromIndex(species, index, spc, mol, atm)
    Type(AtMolCoords), Dimension(:), Intent(in) :: species
    Integer, Intent(in) :: index
    Integer, Intent(out) :: spc, mol, atm
    Integer :: s, m, a
    Integer :: nspc, nmoles, natoms
    ...
    Do s=1,nspc
      ...
      Do m=1,nmoles
        Do a=1, natoms
          If (index==species(s)%index(a,m)) then
            spc=s
            atm=a
            mol=m
            exit
  ...
  End Subroutine config_getspcinfoFromIndex


  !---------------------------------------------------------
  ! Displays by index
  !---------------------------------------------------------
  Subroutine config_displaybyindex(species, nvecs)
    Type(AtMolCoords), Dimension(:), Intent(in) :: species
Integer, Intent(in):: nvecs
    Integer     :: spc, mol, atm, index, unitno
    unitno = 6

    Write(*,*)
    Write(*,*) "DISPLAYING BY INDEX:.............."

    Do index=1,nvecs

Call      config_getspcinfoFromIndex(species, index, spc, mol, atm)
      Write(*,'(a,i5,a,3i2)') "INDEX: ", index, " spc/mol/atm: ", spc, mol, atm
      Write(*,'(a,3f15.6)') " COORD RP: ", species(spc)%coords(atm, mol)%rp
      Write(*,'(a,3f15.6)') " COORD R : ", species(spc)%coords(atm, mol)%r
      Write(*,'(a,3f15.6)') " COORD V : ", species(spc)%coords(atm, mol)%v
      Write(*,'(a,f15.6)')  " CHARGE  : ", species(spc)%charges(atm, mol)
      Write(*,'(a,3f15.6)') " FORCE   : ", species(spc)%forces(atm, mol)

    End Do
  End Subroutine config_displaybyindex
  ...
  Subroutine config_config2xyz(species,filename,pbcs,comment)
    ...
    subset=(/0,0,0/)
    n = config_setxyz(species,subset,entries,pbcs)
  ...
  !----------------------------------------------------------------------------
  ! Dumps the configuration in species to an .xyz file 
  ! each molecule is  separte entry
  ! Requires:  species -- species data structure
  !            filename -- output file name
  !            pbcs -- flag indicating use of %rp (w/o PBCs) or %r (w/PBCs)
  !            comment -- optional comment for .xyz file
  !----------------------------------------------------------------------------
  Subroutine config_config2xyzSeparate(species,filename,pbcs,comment)
    Type(AtMolCoords), Dimension(:), Intent(In)    :: species
    Character(*), Intent(In)                       :: filename
    Logical, Intent(In)                            :: pbcs
    Character(*), Optional                         :: comment

    Integer                                 :: n,error,size,spc,high, m, a
    Integer                                 :: natoms,nmoles, unitno, atype
    Character(len=strlen) :: symbol
    ...
    Do spc = 1,molecules_getnsorbs()
      ...
      Do m=1,nmoles
        Write(unitno,*) natoms
        Write(unitno,*) comment 
        Do a=1,natoms
          atype=config_getatype(species, spc, m, a)
          symbol=atom_getsymbol(atype)
          if (pbcs) then
            Write(unitno,'(a,3f20.10)') Trim(symbol), species(spc)%coords(a,m)%r
          else
            Write(unitno,'(a,3f20.10)') Trim(symbol), species(spc)%coords(a,m)%rp
          endif
        end do
      end do
    End Do

  End Subroutine config_config2xyzSeparate
    ...
    Do spc = lospc,hispc
      ...
      Do mol = lomol,himol
        ...
        Do atm = loatm,hiatm
          ...
          tempset=(/spc,mol,atm/)
          If (pbcs) Then
            entries(n) = visxyz_make(config_getr(species,tempset), &
                atom_getsymbol(atype))
          Else
            entries(n) = visxyz_make(config_getrp(species,tempset), &
                atom_getsymbol(atype))
          End If
        End Do
  ...
  !----------------------------------------------------------------------------
  ! rp is decided such that  distance from al atoms to 1st atom 
  ! are minimized
  !----------------------------------------------------------------------------
  Subroutine config_r2rp(species, scell)
    Type(AtMolCoords), Dimension(:), Intent(InOut)    :: species
    Type(SimCell_Params), Intent(In)         :: scell
Integer :: nspc, spc, nmoles, m, natoms, a
Type(vecType) :: pivot, sepvec
Real(rdbl) :: tempdist2
...
natoms=molecules_getnatoms(spc)
Do m=1,nmoles
pivot=species(spc)%coords(1,m)%r
species(spc)%coords(1,m)%rp = species(spc)%coords(1,m)%r
if (natoms==1) cycle
Do a=2,natoms
Call simcell_minimage(scell, species(spc)%coords(a,m)%r, pivot, sepvec, tempdist2)
species(spc)%coords(a,m)%rp = pivot + sepvec
end do
end do
end do
  End Subroutine config_r2rp
